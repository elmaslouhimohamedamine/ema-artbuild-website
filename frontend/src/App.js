...
function App() {
  ...
  return <main className={`app locale-${locale}`} dir={locale === "ar" ? "rtl" : "ltr"}>
    ...
    <section id="contact" className="contact section-shell"><div className="quote-assistant-grid"><MotionReveal delay={.05}><ChatAssistant mode="embedded" locale={locale} onQuoteClick={() => document.getElementById("quote-form")?.scrollIntoView({ behavior: "smooth", block: "center" })} /></MotionReveal><MotionReveal delay={.12}><div id="quote-form"><QuoteForm t={t} locale={locale} /></div></MotionReveal></div><div className="contact-cards"><a href={`mailto:${CONTACT.email}`} data-testid="contact-email-link"><Mail /><span>Email</span><strong>{CONTACT.email}</strong></a><div data-testid="contact-phones-card"><Phone /><span>Téléphone</span><strong>{CONTACT.phone}<br />{CONTACT.phoneSecondary}</strong></div><div data-testid="contact-location-card"><MapPin /><span>Zone</span><strong>Rabat · Maroc</strong></div></div><nav className="social-links" aria-label="Réseaux sociaux EMA ARTBUILD" data-testid="social-links">{SOCIALS.map(([name, url]) => <a key={name} href={url} target="_blank" rel="noopener noreferrer" data-testid={`social-${name.toLowerCase()}-link`}>{name}<ArrowUpRight size={14} /></a>)}</nav></section>
    <ChatAssistant locale={locale} onQuoteClick={() => goTo("contact")} /><button className="whatsapp-float" onClick={whatsappAction} data-testid="floating-whatsapp-button" aria-label={t.whatsapp}>WA<span>↗</span></button>
